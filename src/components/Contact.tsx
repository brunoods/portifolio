  ...
  return (
    <>
      <Tilt options={tiltOptions}>
        ...
        >
          {/* 3. Lógica para aplicar o efeito de brilho correto */}
          {theme === 'dark' ? (
            <div className="absolute inset-0 bg-[linear-gradient(110deg,#0A0A0A,45%,#1e293b,55%,#0A0A0A)] bg-[length:200%_100%] animate-shimmer opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
          ) : (
            <div className="absolute inset-0 bg-[linear-gradient(110deg,#f5f5f5,45%,#e5e7eb,55%,#f5f5f5)] bg-[length:200%_100%] animate-shimmer opacity-0 group-hover:opacity-50 transition-opacity duration-500" />
          )}
